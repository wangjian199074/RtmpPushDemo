...
public class MainActivity extends Activity {
	...
	public void InitCamera() {
		...
		showlog("原始相机预览尺寸，Original Width:" + prevewSize.width + ", height:" + prevewSize.height);

		List<Size> PreviewSizeList = p.getSupportedPreviewSizes();
		showlog("相机支持的预览尺寸，Listing all supported preview sizes");
		for (Camera.Size size : PreviewSizeList) {
			showlog("  w: " + size.width + ", h: " + size.height);
		}

		List<Integer> PreviewFormats = p.getSupportedPreviewFormats();
		showlog("相机支持的预览格式，Listing all supported preview formats");
		Integer iNV21Flag = 0;
		Integer iYV12Flag = 0;
	...
	private void InitAudioRecord() {
		_iRecorderBufferSize = AudioRecord.getMinBufferSize(SAMPLE_RATE_DEF,
				...
				AudioFormat.ENCODING_PCM_16BIT); //获取最小缓冲区大小
		_AudioRecorder = new AudioRecord(AudioSource.MIC,
				SAMPLE_RATE_DEF, AudioFormat.CHANNEL_IN_STEREO,
	...
	private static String WakeLockTag = "WakeLockTag";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		_wakeLock = pm.newWakeLock(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, WakeLockTag);
	}
